...
| 5️⃣ 儲存/匯出 | 「儲存」→ PDF / 「匯出」→ 選擇格式（PDF、PNG、JPG） |
...
**修復與改進**：
- ✅ 修復標註繪製順序問題：新標註正確顯示在最上層
- ✅ 修復「處理中」覆蓋層頻繁出現的問題
- ✅ 修復圓形匯出異常：編輯器與匯出結果一致（正圓形）
- ✅ 修復開啟新檔案無反應的問題（重置檔案輸入）
- ✅ 新增匯出格式選擇：支援 PDF、PNG、JPG（彈出視窗選擇）
- ✅ 優化側邊欄「新增頁面」按鈕：固定頂部，不被頁面縮圖遮擋
- ✅ 動態版本號顯示：右下角自動顯示當前版本
- ✅ 移除無用的「符合頁面」按鈕
- ✅ 改進記憶體管理：使用 LRU 快取管理圖片資源
- ✅ 提升類型安全：完善 TypeScript 類型定義
- ✅ 效能優化：使用 requestAnimationFrame 節流渲染
